...
function ProjectVideo({ project }: { project: Project }): JSX.Element | null {
  ...
  return (
    <div className="relative overflow-hidden rounded-lg border border-cyan-300/20 bg-black shadow-[0_24px_80px_rgba(34,211,238,0.14)]">
      <video
...
function ProjectCard({ project, featured }: { project: Project; featured: boolean }): JSX.Element {
  return (
    <article
      className={`group relative overflow-hidden rounded-lg border border-white/10 bg-slate-950/80 p-5 shadow-[0_20px_70px_rgba(0,0,0,0.38)] ring-1 ring-purple-400/10 transition duration-300 hover:-translate-y-1 hover:border-cyan-300/35 ${
      ...
      <div className="space-y-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="space-y-2">
            <span className="inline-flex rounded-md border border-purple-300/25 bg-purple-300/10 px-3 py-1 text-xs font-bold uppercase text-purple-200">
              {project.tag}
            </span>
        ...
        {project.metrics ? (
          <div className="flex flex-wrap gap-2">
            {project.metrics.map((metric) => (
              <span
                key={metric}
                className="rounded-md border border-cyan-300/20 bg-cyan-300/10 px-3 py-1 text-xs font-semibold text-cyan-100"
              >
                {metric}
              </span>
            ))}
        ...
        <div className="flex flex-wrap gap-3 pt-1">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex min-h-12 items-center justify-center gap-2 rounded-md border border-cyan-300 bg-cyan-300 px-4 font-extrabold text-slate-950 transition hover:-translate-y-0.5 hover:bg-transparent hover:text-cyan-200"
          >
          ...
          {project.secondaryUrl && project.secondaryLabel ? (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex min-h-12 items-center justify-center gap-2 rounded-md border border-purple-300/40 bg-purple-300/10 px-4 font-extrabold text-purple-100 transition hover:-translate-y-0.5 hover:border-purple-200 hover:bg-purple-300 hover:text-slate-950"
            >
...
function Projects(): JSX.Element {
  ...
  return (
    <SectionShell id="projects" tone="base">
      <SectionHeading
      ...
      <div className="flex flex-wrap items-center justify-center gap-3">
        {tabs.map((tab) => (
          <button
            ...
            className={`rounded-md border px-5 py-2 text-sm font-bold transition ${
              activeTab === tab.id
